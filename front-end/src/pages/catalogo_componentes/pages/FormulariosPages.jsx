...
export default function FormulariosPage({ datos, filtro, favoritos, toggleFav }) {
  ...
  // Filtrado de datos (Categoría Formularios + Buscador)
  const formulariosFiltrados = datos.filter((item) => item.categoria === "formularios" && normalizar(item.titulo).includes(normalizar(filtro)));

  return (
    <>
      {/* ESTO CAMBIA EL TÍTULO DE LA PESTAÑA */}
      <title>Formularios | El Rincón del Front-end</title>
      <meta name="description" content="Colección de login muy creativos e interactivos, hechos con HTML, CSS, JS, listos para usar" />
      ...
      <div className="w-[95%] max-w-[1300px] mx-auto pt-[100px] pb-[50px] grid grid-cols-[repeat(auto-fit,minmax(300px,1fr))] justify-center gap-[40px] cursor-[url('/cursor.svg')_16_16,_auto] [&_input]:cursor-[url('/pointer.svg')_16_16,_text]">
        ...
        {formulariosFiltrados.length === 0 && <p className="text-[var(--text-primary)] text-center w-full">No se encontraron formularios con ese nombre.</p>}
      </div>
    </>
  );
}
