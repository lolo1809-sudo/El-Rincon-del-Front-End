...
export default function CardsPage({ datos, filtro, favoritos, toggleFav }) {
  ...
  // Filtrado de datos (Categoría Cards + Buscador)
  const cardsFiltrados = datos.filter(
    (item) =>
      item.categoria === "cards" &&
      normalizar(item.titulo).includes(normalizar(filtro)),
  );

  return (
    <>
      {/* ESTO CAMBIA EL TÍTULO DE LA PESTAÑA */}
      <title>Cards | El Rincón del Front-end</title>
      <meta
        name="description"
        content="Colección de tarjetas muy creativas hechas con HTML, CSS, JS, listos para usar"
      />
      ...
      <div className="w-[95%] max-w-[1300px] mx-auto pt-[100px] pb-[50px] grid grid-cols-[repeat(auto-fit,minmax(300px,1fr))] justify-center gap-[40px] cursor-[url('/cursor.svg')_16_16,_auto] [&_input]:cursor-[url('/pointer.svg')_16_16,_text]">
                ...
                <div className="w-full flex flex-col items-center justify-center gap-[30px] py-[10px] cursor-[url('/pointer.svg')_16_16,_pointer]">
                  ...
                  <div className="absolute top-[10px] right-[10px] z-10">
                    <BotonFavorito
                      ...
                      aria-label={`Añadir ${item.titulo} a favoritos`}
                    />
                  </div>
                  ...
                  <div>
                    ...
                    >
                      <i
                        aria-hidden="true"
                        className="fa-solid fa-gamepad text-white cursor-[url('/pointer.svg')_16_16,_pointer]"
                      ></i>
                      Ver Demo
                    </a>
                  </div>
                  ...
                  >
                    <i
                      aria-hidden="true"
                      className="fa-solid fa-code cursor-[url('/pointer.svg')_16_16,_pointer]"
                    ></i>
                    Ver Repositorio
                  </a>
                </div>
        ...
        {cardsFiltrados.length === 0 && (
          <p className="text-[var(--text-primary)] text-center w-full">
            No se encontraron tarjetas con ese nombre.
          </p>
        )}
      </div>
    </>
