...
export default function NavegacionPage({ datos, filtro, favoritos, toggleFav }) {
  ...
  // Filtrado de datos (Categoría Navegacion + Buscador)
  const navegacionFiltrados = datos.filter((item) => item.categoria === "navegacion" && normalizar(item.titulo).includes(normalizar(filtro)));

  return (
    <>
      {/* ESTO CAMBIA EL TÍTULO DE LA PESTAÑA */}
      <title>Navegación | El Rincón del Front-end</title>
      <meta name="description" content="Colección de estructuras de Navegación hechos con HTML, CSS, JS, listos para usar" />
      ...
      <div className="w-[95%] max-w-[1300px] mx-auto pt-[100px] pb-[50px] grid grid-cols-[repeat(auto-fit,minmax(300px,1fr))] justify-center gap-[40px] cursor-[url('/cursor.svg')_16_16,_auto] [&_input]:cursor-[url('/pointer.svg')_16_16,_text]">
        ...
        {navegacionFiltrados.length === 0 && <p className="text-[var(--text-primary)] text-center w-full">No se encontraron estructuras de navegación con ese nombre.</p>}
      </div>
    </>
  );
}
